...
import CenterImage from '@site/static/js/CenterImage'

## 介绍一下 Java 中的异常？

`Throwable` 可以用来表示任何可以作为异常抛出的类，分为 `Error` 和 `Exception` 两种类型。

<CenterImage atl="异常">https://i.loli.net/2021/07/31/kyXmHLnpdaj7vQw.png</CenterImage>

<Tabs
    defaultValue="Error-错误"
    values={[
        {label: 'Error-错误', value: 'Error-错误'},
        {label: 'Exception-异常', value: 'Exception-异常'}
]}>

<TabItem value="Error-错误">

`Error` 表示 JVM 无法处理的错误，如：`OutOfMemoryError`，`StackOverflowError` 等。

</TabItem>

<TabItem value="Exception-异常">
...
:::caution 在 finally 中修改返回值对应的变量的的值并不会影响返回值
